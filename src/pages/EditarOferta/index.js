...
export default function AdicionarOferta(props) {
  ...
  useEffect(() => {
    db.collection('ofertas').doc(props.match.params.id).onSnapshot((consulta)=>{
        if(consulta.exists) {        
        setOferta(consulta.data())
        setLoaded(true)
        }
    })    
  }, []);
    ...
    <LayoutAdmin>
      {loaded ? 
        ...
        <Form id="form" onSubmit={handleSubmit}>
          ...
          <Button variant="btn-block btn-large btn-warning" type="submit">
            Editar a Oferta
          </Button>
        </Form>
      </div>
